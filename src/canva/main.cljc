(ns canva.main
  "Kotodama WASM entrypoint for the Canva clean-room actor (L4) — Clojure port.
  ...
  facts (`canva.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Design"        :plural "designs"        :id-prefix "canva_des"
    :fields [:title :pageCount]                :required [:title]
    :coerce {:pageCount :int}                  :refs {}}
   {:entity "Asset"         :plural "assets"         :id-prefix "canva_ass"
    :fields [:name :type]                      :required [:name]
    :coerce {}                                 :refs {}}
   {:entity "Folder"        :plural "folders"        :id-prefix "canva_fol"
    :fields [:name]                            :required [:name]
    :coerce {}                                 :refs {}}
   {:entity "BrandTemplate" :plural "brandtemplates" :id-prefix "canva_bra"
    :fields [:title :viewUrl]                  :required [:title]
    :coerce {}                                 :refs {}}
   {:entity "ExportJob"     :plural "exportjobs"     :id-prefix "canva_exp"
    :fields [:status :format]                  :required [:status]
    :coerce {}                                 :refs {}}
   {:entity "User"          :plural "users"          :id-prefix "canva_use"
    :fields [:displayName]                     :required [:displayName]
    :coerce {}                                 :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"canva.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "canva-compat" :tier tier :entities entities} 200])
